...
def enviar_a_webhook_se47(datos_payload):
    """
    Envía el payload JSON al webhook configurado para SE47.
    """
    webhook_url = current_app.config.get('WEBHOOK_SE47_URL')
    webhook_token = current_app.config.get('WEBHOOK_SE47_AUTH')

    if not webhook_url or not webhook_token:
        current_app.logger.error("Webhook SE47 URL o AUTH no configurados en app.config.")
...
@se47_bp.route('/api/empleado', methods=['GET'])
def api_empleado():
...
@se47_bp.route('/api/trabajo/<trabajo_id>', methods=['GET'])
...
@se47_bp.route('/', methods=['GET','POST'])
def sellado_form_se47():
    ...
    if request.method == 'POST':
        ...
        if validation_errors:
            ...
            return render_template(
                'processes/sellado/sellado_form_se47.html',
                nombre_proceso="Sellado", subseccion="Formulario SE47",
        ...
        payload = {
            ...
            "fotocelda_otro": datos.get('fotocelda_otro'),
        ...
        current_app.logger.info("JSON del Payload del Formulario SE47 (Simulado):")
        current_app.logger.info(payload)

        # --- ENVÍO DE DATOS AL WEBHOOK ---
        webhook_result = enviar_a_webhook_se47(payload)
        ...
        return redirect(url_for('se47.sellado_form_se47'))
    ...
    return render_template(
        ...
        subseccion="Formulario SE47 (FORMATO EN PRUEBA)", 
