...
def enviar_a_webhook_se26(datos_payload):
    """
    Envía el payload JSON al webhook configurado para SE26.
    """
    webhook_url = current_app.config.get('WEBHOOK_SE26_URL')
    webhook_token = current_app.config.get('WEBHOOK_SE26_AUTH')

    if not webhook_url or not webhook_token:
        current_app.logger.error("Webhook SE26 URL o AUTH no configurados en app.config.")
...
@se26_bp.route('/api/empleado', methods=['GET'])
def api_empleado():
...
@se26_bp.route('/api/trabajo/<trabajo_id>', methods=['GET'])
...
@se26_bp.route('/', methods=['GET','POST'])
def sellado_form_se26():
    ...
    if request.method == 'POST':
        ...
        if validation_errors:
            ...
            return render_template(
                'processes/sellado/sellado_form_se26.html',
                nombre_proceso="Sellado", subseccion="Formulario SE26",
        ...
        current_app.logger.info("JSON del Payload del Formulario SE26 (Simulado):")
        current_app.logger.info(payload)

        # --- ENVÍO DE DATOS AL WEBHOOK ---
        webhook_result = enviar_a_webhook_se26(payload)
        ...
        return redirect(url_for('se26.sellado_form_se26'))

    # GET: Mostrar formulario vacío y fecha/hora de Bogotá en el campo
    return render_template(
        'processes/sellado/sellado_form_se26.html',
        nombre_proceso="Sellado", 
        subseccion="Formulario SE26 (FORMATO EN PRUEBA)", 
